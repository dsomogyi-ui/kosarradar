...
export async function GET(){try{
 ...
 return reply({user:{id:auth.user.id,email:auth.user.email||'',displayName:data?.display_name||'',favoriteShopIds:(data?.favorite_shop_ids||[]).filter(allowedFavoriteId)}});
...
export async function PUT(request:Request){try{
 ...
 const shops=await getShops();if(data.favoriteShopIds.some(id=>!shops.shops.some(s=>s.id===id)))return reply({error:'Csak Auchan és Tesco üzlet menthető.'},400);
