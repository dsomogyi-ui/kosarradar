...
export type BasketSnapshot={name:string;lines:ShoppingLine[];area:Area;loyalty:string[];extraStopCost:number};
...
export function snapshotInput(value:unknown):BasketSnapshot|null{
 try{
  ...
  return {name:d.name.trim(),lines,area:{place:d.area.place?{id:d.area.place.id,name:d.area.place.name,region:d.area.place.region,latitude:d.area.place.latitude,longitude:d.area.place.longitude}:null,radius:d.area.radius},loyalty:settings.loyalty,extraStopCost:settings.extraStopCost};
 }catch{return null;}
}
