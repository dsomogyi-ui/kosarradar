export type MasterProduct = {
  ...
  image_front_small_url?:string; source:"kosarradar"; keywords?:string[];
};
export const PRODUCT_MASTER:MasterProduct[]=[
 ...
 {code:"KR-MERIDOL-75",product_name:"Meridol Gum Protection",brands:"Meridol",quantity:"75 ml",source:"kosarradar",keywords:["meridol","fogkrém"]},
...
export function searchMaster(q:string){
 ...
 return PRODUCT_MASTER.filter(p=>[p.product_name,p.brands,p.quantity,...(p.keywords||[])]
   .filter(Boolean).join(" ").toLocaleLowerCase("hu-HU").includes(n));
}
